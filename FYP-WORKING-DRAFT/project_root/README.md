...
## Ubuntu Lab Reliability

For the Ubuntu + Kali VM lab, the host-only interface should stay pinned to:

- interface: `enp0s8`
- Ubuntu IP: `192.168.56.101`
- Kali IP: `192.168.56.102`

Reusable Ubuntu deployment assets now live in [`deployment/ubuntu/`](./deployment/ubuntu):

- `run_inference.sh` restores the host-only IP if needed and starts `FYP/inference.py`
- `elai-inference.env` stores the lab runtime variables
- `elai-inference.service` lets Ubuntu auto-start inference with `systemd`
- `01-elai-hostonly-enp0s8.yaml` pins the host-only IP through Netplan

See [`deployment/UBUNTU_KALI_LAB_SETUP.md`](./deployment/UBUNTU_KALI_LAB_SETUP.md) for the
copy-and-enable commands.

## Edge Deployment Path

The project now supports an edge-enforced blocking model:

1. `inference.py` runs on the monitored edge machine.
2. Alerts are sent to the central dashboard on Ubuntu.
3. The analyst reviews the alert in the dashboard.
4. A block request is sent to `FYP/edge_block_agent.py` on the monitored device.
5. The edge device applies a local firewall drop rule for that attacker IP.

Use [`deployment/EDGE_VM_SETUP.md`](./deployment/EDGE_VM_SETUP.md) to extend the lab from
`Kali + Ubuntu` to `Kali + Edge VM + Ubuntu`.
