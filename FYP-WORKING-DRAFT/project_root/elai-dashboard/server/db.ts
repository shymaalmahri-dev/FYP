...
export async function markAlertsBlockedBySource(ipAddress: string) {
  const db = await getDb();
  if (!db) return;
  ...
  await db
    ...
    .where(eq(alerts.sourceIp, ipAddress));
}

export async function markAlertsUnblockedBySource(ipAddress: string) {
  const db = await getDb();
  if (!db) return;
