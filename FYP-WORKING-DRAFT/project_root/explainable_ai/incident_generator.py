...
def build_prompt(attack_type, attacker_ip, features, verbosity="brief"):
    ...
    event_noun = "event"
    event_focus = "suspicious activity"
    analyst_goal = "why this activity was detected and what it means operationally"

    if attack_type == "Normal_Traffic":
        event_focus = "sampled benign traffic"
        analyst_goal = "why the model treated this traffic as benign and why it was retained for validation"
    elif attack_type == "Gray_Zone_Review":
        event_focus = "gray-zone traffic"
        analyst_goal = "why the event was placed into analyst review instead of being auto-blocked"
    else:
        event_noun = "attack"
        event_focus = "malicious activity"
        analyst_goal = "why this attack was detected and what behavior indicates malicious activity"

    if verbosity == "brief":
        return f"""
You are a cybersecurity SOC analyst.

Event Type: {attack_type}
...
Write a brief 2-3 sentence explanation of {analyst_goal}.
Focus on the features and their significance for {event_focus}.
""".strip()
...
Event Type: {attack_type} - {context['name']}
Attacker IP: {attacker_ip}
...
1. Why was this {event_noun} or review state detected (behavioral analysis)?
...
def build_fallback_report(attack_type, attacker_ip, features, verbosity="detailed"):
    ...
    if attack_type == "Gray_Zone_Review":
        event_label = "gray-zone event"
    elif attack_type == "Normal_Traffic":
        event_label = "normal validation sample"
    top_features = ", ".join(
    ...
    if verbosity == "brief":
        if top_features:
            return (
                f"{attack_type} {event_label} from {attacker_ip} was flagged because the model observed indicators such as "
                f"{top_features}. This pattern aligns with {context['description'].lower()}."
            )
        return (
            f"{attack_type} {event_label} from {attacker_ip} was flagged based on the observed traffic behavior. "
